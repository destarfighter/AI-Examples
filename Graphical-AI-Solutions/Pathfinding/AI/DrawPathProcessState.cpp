...
void DrawPathProcessState::update(float deltaTime_) {
...
void DrawPathProcessState::enter() {
	// initialize PathAnimater
	pathAnimater_ptr_->initialize(CHARACTER_SPRITESHEET_FILE, search_);
	// add pathAnimater to WorldState
	auto worldState = WorldStateLocator::getWorldState();
	worldState->addNewObject({ "pathAnimation", 5 }, pathAnimater_ptr_, true, true);
}

DrawPathProcessState::DrawPathProcessState(AIController* parent, AnimatedSprite* owner, std::vector<unsigned int long> path, std::vector<TileFrame> search, bool foundPath) 
